...
def loginprocess(request):
    ...
    # Case : password does not typed
        ...
        messages.error(request, "Enter your password",
                       extra_tags="login_password_error")
        return redirect("/")
    ...
    # Case : email - not exists in database
        ...
        messages.error(request, "Email is not in database",
        ...
        if not bcrypt.checkpw(request.POST["login_password"].encode(), User.objects.get(email=request.POST["login_email"]).password.encode()):
            messages.error(
                request, "Wrong password, please check your password again", extra_tags="login_error")
            return redirect("/")
...
def book(request):
    ...
    if request.session["login_status_and_id"]["status"] == False:
        messages.error(request, "Login Fail, please login again!",
                       extra_tags="login_fail_error")
        return redirect("/")
    else:
        context = {
...
def addbookandreviewprocess(request):
    ...
    if request.session["login_status_and_id"]["status"] == False:
        ...
        if len(request.POST["title"]) < 1:
            messages.error(
                request, "Please enter your book title (Not accepted blank book title", extra_tags="title_error")
            ...
            # Create review
            Review.objects.create(rating=request.POST["rating"], review=request.POST["review"], user=User.objects.get(
                id=request.session["login_status_and_id"]["login_id"]), book=book)
...
def addreviewprocess(request, book_id):
    user = User.objects.get(
        id=request.session["login_status_and_id"]["login_id"])
